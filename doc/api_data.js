define({ "api": [
  ...
  },
  {
    ...
    "name": "Login",
    "group": "Web",
    "description": "<p>Send login push notification to one of mobile app. The notification contains data: <code>{\"type\": \"WEB_LOGIN_REQUEST\", \"requestingAppId\": \"APP_ID\"}</code> Where <code>type</code> always be <code>WEB_LOGIN_REQUEST</code>,  and <code>requestingAppId</code> is the ID of the app that requests login. Since it can be called multiple time, it can also be used to validate login status (from partner web server)</p>",
    "parameter": {
      "fields": {
        "Parameter": [
          {
