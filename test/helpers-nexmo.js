...
describe("Nexmo Helpers", () => {
    ...
    it("should send verification sms properly", async () => {
        ...
        nock(config.NEXMO_REST_URL)
            .post(
                `/sms/json`,
                `from=humanID&text=${encodeURIComponent(text)}&to=${number}&api_key=${
    ...
    it("should send request phone verification properly", async () => {
        ...
        nock(config.NEXMO_API_URL)
            .get(
                `/verify/json?api_key=${config.NEXMO_API_KEY}&api_secret=${config.NEXMO_API_SECRET}&number=${number}&brand=humanID&code_length=4`
            )
    ...
    it("should check verification code properly", async () => {
        ...
        nock(config.NEXMO_API_URL)
            .get(
                `/verify/check/json?api_key=${config.NEXMO_API_KEY}&api_secret=${config.NEXMO_API_SECRET}&request_id=${requestId}&code=${verificationCode}`
            )
            .reply(200, { status: "0", request_id: requestId });
